...
async def test__reconcile__static_root__converges_the_roots_own_partition_set(
    repo: MagicMock, metadata: MagicMock
) -> None:
    ...
    # Act
    result = await service.reconcile(config)
...
async def test__reconcile__execution_finding__reaches_the_returned_result(repo: MagicMock, metadata: MagicMock) -> None:
