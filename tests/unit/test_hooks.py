"""Unit tests for the aio lifecycle hooks: the no-op base, the protocol and the event."""
...
async def test__base_hooks__every_method__is_a_noop(method: str, event: PartitionEvent) -> None:
    # Arrange
    hooks = BasePartitionLifecycleHooks()

    # Act / Assert -- must not raise
    assert await getattr(hooks, method)(event) is None
...
async def test__base_hooks_subclass__overriding_one_method__the_others_stay_quiet(event: PartitionEvent) -> None:
    # Arrange
    seen: list[str] = []
    ...
    # Act
    await hooks.before_create(event)
    await hooks.before_drop(event)
