"""Unit tests for the aio lifecycle hooks: the no-op base and the protocol."""
...
async def test__base_hooks__before_create__is_noop(config: TablePartitionConfig, partition_info: PartitionInfo) -> None:
    # Arrange
    hooks = BasePartitionLifecycleHooks()

    # Act / Assert -- must not raise
    assert await hooks.before_create(config, partition_info) is None


async def test__base_hooks__after_create__is_noop(config: TablePartitionConfig, partition_info: PartitionInfo) -> None:
    # Arrange
    hooks = BasePartitionLifecycleHooks()

    # Act / Assert -- must not raise
    assert await hooks.after_create(config, partition_info) is None


async def test__base_hooks__before_detach__is_noop(partition_info: PartitionInfo) -> None:
    # Arrange
    hooks = BasePartitionLifecycleHooks()

    # Act / Assert -- must not raise
    assert await hooks.before_detach("events", partition_info) is None


async def test__base_hooks__after_detach__is_noop() -> None:
    # Arrange
    hooks = BasePartitionLifecycleHooks()

    # Act / Assert -- must not raise
    assert await hooks.after_detach("events", "events__2024_01") is None


async def test__base_hooks__before_drop__is_noop() -> None:
    # Arrange
    hooks = BasePartitionLifecycleHooks()

    # Act / Assert -- must not raise
    assert await hooks.before_drop("events", "events__2024_01") is None


async def test__base_hooks__after_drop__is_noop() -> None:
    # Arrange
    hooks = BasePartitionLifecycleHooks()

    # Act / Assert -- must not raise
    assert await hooks.after_drop("events", "events__2024_01") is None
...
async def test__base_hooks_subclass__before_create__receives_the_partition_about_to_be_created(
    ...
    class _Hooks(BasePartitionLifecycleHooks):
        async def before_create(self, cfg: TablePartitionConfig, partition: PartitionInfo) -> None:
            seen.append(partition)

    # Act
    await _Hooks().before_create(config, partition_info)
...
async def test__base_hooks_subclass__overriding_one_method__only_overridden_method_fires(
    config: TablePartitionConfig, partition_info: PartitionInfo
) -> None:
    # Arrange
    called: list[str] = []
    ...
    # Act
    await hooks.before_create(config, partition_info)
    await hooks.before_drop("events", "events__2024_01")
