...
async def test__apply__default_conflict_on_range_attach__moves_rows_and_retries(
    executor: PlanExecutor, repo: MagicMock, metadata: MagicMock
) -> None:
    # Arrange
...
async def test__apply__default_conflict_retries_exhausted__restores_rows_and_raises(
    executor: PlanExecutor, repo: MagicMock, metadata: MagicMock
) -> None:
    # Arrange
...
async def test__apply__cancelled_during_attach_after_reconcile__restores_rows_and_reraises(
    ...
    # Arrange
    repo.attach_partition.side_effect = [_default_conflict(), asyncio.CancelledError()]
    repo.reconcile_default_rows.return_value = 5
    metadata.get_default_partition.return_value = _default_partition()
