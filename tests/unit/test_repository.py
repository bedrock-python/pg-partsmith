...
def _locks(conn: AsyncMock) -> list[str]:
...
async def test__reconcile_and_attach__locks_the_parent_and_both_sides_then_moves_and_attaches() -> None:
    # Arrange
    engine, conn = _engine(_Catalog(moved_rows=7))
    repo = PostgresPartitionRepository(engine)

    # Act
    moved = await repo.reconcile_and_attach(
        "events",
        "events__2024_04",
...
async def test__reconcile_and_attach__composite_key__pads_the_bound_and_keeps_null_keys_in_default() -> None:
    # Arrange
    engine, conn = _engine(_Catalog(moved_rows=1))
    repo = PostgresPartitionRepository(engine)

    # Act
    await repo.reconcile_and_attach(
        "events",
        "events__2024_04",
        RangeBounds(from_value="2024-04-01", to_value="2024-05-01"),
...
async def test__reconcile_and_attach__foreign_default_partition__is_not_locked() -> None:
...
async def test__reconcile_and_attach__empty_key__is_rejected_before_any_sql() -> None:
    # Arrange
    engine, _ = _engine()
    repo = PostgresPartitionRepository(engine)
...
async def test__detach_partition__auto__marks_the_orphan_then_detaches_concurrently() -> None:
    ...
    engine.begin.assert_not_called()
    assert conn.execution_options.await_count == 1
...
async def test__detach_partition__rows_still_referenced__raises_partition_referenced(mode: DetachMode) -> None:
    # Arrange
...
async def test__attach_partition__marked_orphan__loses_the_marker_and_keeps_the_user_comment() -> None:
    # Arrange
    ...
    # Act
    await repo.attach_partition("events", "events__2024_01", HashBounds(modulus=2, remainder=0))
...
async def test__attach_partition__marker_only_comment__is_removed_entirely() -> None:
    # Arrange
...
async def test__attach_partition__no_marker__writes_no_comment() -> None:
    # Arrange
...
async def test__move_rows__cascading_foreign_key__is_refused_before_any_row_moves() -> None:
    # Arrange
...
async def test__reconcile_default_rows__set_null_foreign_key__is_refused() -> None:
    # Arrange
...
async def test__move_rows__identity_always_on_the_target__overrides_the_system_value() -> None:
    # Arrange
...
async def test__detach_partition__expected_oid_differs__is_stale_and_nothing_is_marked() -> None:
    # Arrange
    ...
    with pytest.raises(PlanStaleError, match="OID 9999"):
        await repo.detach_partition("events", "events__2024_01", mode=DetachMode.BLOCKING, expected_oid=4242)
    statements = _statements(conn)
    assert not any("COMMENT ON" in s for s in statements)
    assert not any("DETACH PARTITION" in s for s in statements)


async def test__detach_partition__expected_oid_matches__locks_checks_marks_and_detaches_in_one_transaction() -> None:
    # Arrange
...
async def test__detach_partition__concurrent_form_with_an_oid__pins_verifies_marks_then_detaches() -> None:
    # Arrange
    engine, conn = _engine(_Catalog(oid=4242, attached_to=True))
    repo = PostgresPartitionRepository(engine)

    # Act
    await repo.detach_partition("events", "events__2024_01", mode=DetachMode.CONCURRENT, expected_oid=4242)
...
async def test__detach_partition__concurrent_form__swapped_before_the_pin__is_stale_and_nothing_is_marked() -> None:
...
async def test__detach_partition__no_expected_oid__skips_the_identity_checks() -> None:
    # Arrange
    engine, conn = _engine()
    repo = PostgresPartitionRepository(engine)
...
async def test__drop_partition__drain_into__moves_the_tail_in_the_drop_transaction() -> None:
    # Arrange
    ...
    # Act
    await repo.drop_partition("events__2024_01", drain_into="public.events_flat")
...
async def test__move_rows__referenced_rows__refused_atomically_with_guidance() -> None:
...
async def test__attach_partition__expected_oid_mismatch__fails_inside_the_attach_transaction() -> None:
    ...
    # Act / Assert
    with pytest.raises(PlanStaleError, match="OID 9999"):
        await repo.attach_partition("events", "events__x", HashBounds(modulus=2, remainder=0), expected_oid=4242)
    statements = _statements(conn)
...
async def test__drop_partition__drain_into__returns_the_rows_it_moved() -> None:
    # Arrange
    marked = orphan_table_comment("public.events")
    ...
    # Act / Assert
    assert await repo.drop_partition("events__2024_01", drain_into="public.events_flat") == 3


async def test__detach_partition__blocking_foreign_with_a_swap__rolls_back_on_the_recheck() -> None:
    ...
    # Act / Assert
    with pytest.raises(PlanStaleError, match="OID 9999"):
        await repo.detach_partition("metrics", "metrics__2026_01", mode=DetachMode.BLOCKING, expected_oid=4242)
    assert any("DETACH PARTITION" in s for s in _statements(conn))
    engine.begin.assert_called_once()


async def test__detach_partition__foreign_with_an_oid__uses_the_transactional_blocking_form() -> None:
    ...
    # Act
    await repo.detach_partition("metrics", "metrics__2026_01", mode=DetachMode.AUTO, expected_oid=4242)
...
async def test__move_rows__identity_sequence_with_a_cache__refuses_what_a_session_may_hold() -> None:
...
async def test__move_rows__identity_sequence_with_a_cache_nobody_holds__moves() -> None:
...
async def test__move_rows__cached_identity__values_before_its_start_are_not_refused() -> None:
...
async def test__move_rows__cycling_identity__asks_about_the_whole_range_not_one_residue() -> None:
...
async def test__move_rows__identity_target__keeps_the_ids_it_moved_and_asks_about_those() -> None:
    # Arrange
