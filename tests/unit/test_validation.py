"""Unit tests for the aio ``PartitionValidationService``: the config is checked against the catalog before any DDL."""
...
async def test__validate_config__matching_table__passes(
    validation: PartitionValidationService, metadata: MagicMock
) -> None:
    # Arrange / Act -- must not raise
    await validation.validate_config(_config(schema="public"))

    # Assert
    metadata.get_partition_type.assert_awaited_once_with("public.events")
    metadata.get_partition_columns.assert_awaited_once_with("public.events")
    metadata.get_unique_constraint_columns.assert_not_awaited()


async def test__validate_config__table_not_partitioned__raises(
    validation: PartitionValidationService, metadata: MagicMock
) -> None:
    # Arrange
    ...
    with pytest.raises(InvalidPartitionConfigError, match="'events' is not partitioned"):
        await validation.validate_config(_config())

    metadata.get_partition_columns.assert_not_awaited()


async def test__validate_config__partition_type_mismatch__raises_with_both_types(
    validation: PartitionValidationService, metadata: MagicMock
) -> None:
    # Arrange
...
async def test__validate_config__no_partition_columns__raises(
    validation: PartitionValidationService, metadata: MagicMock
) -> None:
    # Arrange
...
async def test__validate_config__mixed_case_column__raises(
    validation: PartitionValidationService, metadata: MagicMock
) -> None:
    ...
    with pytest.raises(InvalidPartitionConfigError, match=r"\['createdAt'\].*mixed-case"):
        await validation.validate_config(_config(partition_column="createdat"))


async def test__validate_config__single_column_mismatch__keeps_the_historical_wording(
    validation: PartitionValidationService, metadata: MagicMock
) -> None:
    # Arrange
...
async def test__validate_config__composite_key_mismatch__reports_the_whole_key(
    validation: PartitionValidationService, metadata: MagicMock
) -> None:
    # Arrange
    ...
    with pytest.raises(InvalidPartitionConfigError, match=r"Partition key mismatch.*\('created_at', 'tenant_id'\)"):
        await validation.validate_config(_composite_config())


async def test__validate_config__composite_config_on_a_single_column_table__reports_the_whole_key(
    validation: PartitionValidationService, metadata: MagicMock
) -> None:
    # Arrange
...
async def test__validate_config__composite_key_in_catalog_order__passes(
    validation: PartitionValidationService, metadata: MagicMock
) -> None:
    # Arrange
...
async def test__validate_config__expression_key__error_propagates_as_invalid_config(
    validation: PartitionValidationService, metadata: MagicMock
) -> None:
...
async def test__validate_config__nested_level_missing_from_a_constraint__raises_with_a_fix(
    validation: PartitionValidationService, metadata: MagicMock
) -> None:
    # Arrange
    ...
    with pytest.raises(InvalidPartitionConfigError) as excinfo:
        await validation.validate_config(_nested_config(HashPartitioning(key="tenant_id", modulus=4)))
...
async def test__validate_config__nested_level_present_in_every_constraint__passes(
    validation: PartitionValidationService, metadata: MagicMock
) -> None:
    # Arrange
...
async def test__validate_config__no_unique_constraints__passes(
    validation: PartitionValidationService, metadata: MagicMock
) -> None:
    # Arrange
...
async def test__validate_config__composite_nested_key__every_column_is_checked(
    validation: PartitionValidationService, metadata: MagicMock
) -> None:
    ...
    with pytest.raises(InvalidPartitionConfigError, match="'shard_id'"):
        await validation.validate_config(_nested_config(HashPartitioning(key=("tenant_id", "shard_id"), modulus=4)))


async def test__validate_config__list_level_missing_from_a_constraint__raises(
    validation: PartitionValidationService, metadata: MagicMock
) -> None:
    # Arrange
    ...
    with pytest.raises(InvalidPartitionConfigError, match="'region'"):
        await validation.validate_config(_nested_config(level))


async def test__validate_config__deeper_levels__are_checked_too(
    validation: PartitionValidationService, metadata: MagicMock
) -> None:
    # Arrange
    metadata.get_unique_constraint_columns.return_value = (("id", "created_at", "tenant_id"),)
