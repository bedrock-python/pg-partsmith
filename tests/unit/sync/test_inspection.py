"""Unit tests for the sync ``PartitionInspector``: what is measured, and how the planning context is built."""
...
def test__inspect__measure_false__returns_the_tree_untouched(
    ...
    # Act
    result = inspector.inspect(_config(_size_policy()), measure=False)

    # Assert
    assert result is tree
    metadata.get_actual_tree.assert_called_once_with("events")
    metadata.measure.assert_not_called()


def test__inspect__table_not_partitioned__returns_none_without_measuring(
    inspector: PartitionInspector, metadata: MagicMock
) -> None:
    # Arrange
    ...
    # Act / Assert
    assert inspector.inspect(_config(_size_policy())) is None
    metadata.measure.assert_not_called()


def test__inspect__scheme_without_a_progression_level__never_measures(
    inspector: PartitionInspector, metadata: MagicMock
) -> None:
...
def test__inspect__policy_that_needs_no_facts__never_measures(
    inspector: PartitionInspector, metadata: MagicMock
) -> None:
    # Arrange
    ...
    # Act
    inspector.inspect(_config(LifecyclePolicy(retention=KeepNewest(count=3))))

    # Assert
    metadata.measure.assert_not_called()


def test__inspect__policy_needs_facts__measures_the_progression_members_and_orphans(
    inspector: PartitionInspector, metadata: MagicMock
) -> None:
    # Arrange
    ...
    # Act
    result = inspector.inspect(_config(policy))

    # Assert
    assert result is measured
    metadata.measure.assert_called_once_with(
        tree,
...
def test__inspect__default_partition__is_never_a_fact_target(
    inspector: PartitionInspector, metadata: MagicMock
) -> None:
    # Arrange
    ...
    # Act
    inspector.inspect(_config(_size_policy()))

    # Assert
    assert metadata.measure.call_args.kwargs["targets"] == ("events__2024_02",)


def test__inspect__nothing_to_measure__returns_the_tree_without_a_query(
    inspector: PartitionInspector, metadata: MagicMock
) -> None:
    ...
    # Act
    result = inspector.inspect(_config(_size_policy()))
...
def test__inspect__nested_progression_level__measures_members_below_the_root(
    inspector: PartitionInspector, metadata: MagicMock
) -> None:
...
def test__context__naive_instant__is_read_as_utc(inspector: PartitionInspector) -> None:
    # Arrange / Act
    context = inspector.context(_config(), now=NOW.replace(tzinfo=None))

    # Assert
    assert context.now == NOW


def test__context__aware_instant__is_kept_as_is(inspector: PartitionInspector) -> None:
    # Arrange
    moscow = NOW.astimezone(ZoneInfo("Europe/Moscow"))

    # Act
    context = inspector.context(_config(), now=moscow)

    # Assert
    assert context.now is moscow


def test__context__mode_and_windows__are_passed_through(inspector: PartitionInspector) -> None:
    # Arrange
    windows = {"created_at": (Window(start=datetime(2024, 1, 1, tzinfo=UTC), end=datetime(2024, 2, 1, tzinfo=UTC)),)}

    # Act
    context = inspector.context(_config(), now=NOW, mode=PlanMode.EXPLICIT, explicit_windows=windows)
...
def test__context__time_axis__reads_no_cursor(inspector: PartitionInspector, metadata: MagicMock) -> None:
    # Arrange / Act
    inspector.context(_config(), now=NOW)

    # Assert
    metadata.get_key_high_water_mark.assert_not_called()


def test__context__integer_axis__reads_the_high_water_mark(inspector: PartitionInspector, metadata: MagicMock) -> None:
    # Arrange
    config = TablePartitionConfig(
        table_name="queue",
        schema="public",
    ...
    # Act
    context = inspector.context(config, now=NOW)

    # Assert
    metadata.get_key_high_water_mark.assert_called_once_with("public.queue", "msg_id", sequence=False)
    assert context.cursors == {"msg_id": 4321}


def test__context__sequence_cursor_source__asks_for_the_sequence(
    inspector: PartitionInspector, metadata: MagicMock
) -> None:
    # Arrange
    config = TablePartitionConfig(
    ...
    # Assert
    metadata.get_key_high_water_mark.assert_called_once_with("queue", "msg_id", sequence=True)
    assert context.cursors == {"msg_id": 10}


def test__context__empty_table__records_no_cursor(inspector: PartitionInspector, metadata: MagicMock) -> None:
    # Arrange
    config = TablePartitionConfig(
...
def test__context__integer_axis_below_a_hash_root__is_still_read(
    inspector: PartitionInspector, metadata: MagicMock
) -> None:
    # Arrange
    config = TablePartitionConfig(
...
def test__context__hash_root_alone__reads_nothing(inspector: PartitionInspector, metadata: MagicMock) -> None:
    # Arrange
