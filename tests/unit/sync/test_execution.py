...
def repo() -> MagicMock:
    ...
    repo.create_foreign_table_like = MagicMock(side_effect=lambda *_args, **_kwargs: next(oids))
...
@pytest.mark.parametrize("sqlstate", ["42P07", "42710", "42809"])
def test__apply__driver_error_attach_conflict__is_benign_like_a_sqlalchemy_one(
    executor: PlanExecutor, repo: MagicMock, metadata: MagicMock, sqlstate: str
) -> None:
    # Arrange
...
def test__apply__driver_error_default_conflict__moves_rows_and_retries(
    executor: PlanExecutor, repo: MagicMock, metadata: MagicMock
) -> None:
    # Arrange
...
def test__apply__driver_error_on_attach__propagates_after_restoring_rows(
    executor: PlanExecutor, repo: MagicMock, metadata: MagicMock
) -> None:
    # Arrange
    repo.attach_partition.side_effect = _driver_default_conflict()
...
def test__apply__repository_error_without_a_sqlstate__propagates_after_restoring_rows(
    executor: PlanExecutor, repo: MagicMock, metadata: MagicMock
) -> None:
...
def test__apply__attach_target_swapped_during_the_bulk_reconcile__fails_closed(
    executor: PlanExecutor, repo: MagicMock, metadata: MagicMock
) -> None:
    ...
    repo.reconcile_and_attach.assert_not_called()
    assert repo.reconcile_default_rows.call_count == 1
    logger.warning.assert_called_once()


def test__apply__tail_move_refused_inside_the_locked_attach__rows_go_back_and_it_is_an_issue(
    executor: PlanExecutor, repo: MagicMock, metadata: MagicMock
) -> None:
...
def test__create_partition__existing_relation_vanished_before_recovery__is_stale(
    executor: PlanExecutor, repo: MagicMock, metadata: MagicMock
) -> None:
