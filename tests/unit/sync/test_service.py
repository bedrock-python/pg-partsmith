...
def repo() -> MagicMock:
    ...
    oids = count(101)
    repo.create_table_like = MagicMock(side_effect=lambda *_args, **_kwargs: next(oids))
...
def test__apply__holds_the_table_lock_while_the_executor_runs(
    ...
    service._executor.apply.assert_called_once_with(config, plan, continue_on_error=True, allow_config_drift=False)
