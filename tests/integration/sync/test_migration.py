...
def _two_identity_destination(engine: Engine, table: str) -> str:
...
def test__unpartition__a_later_identity_refuses__the_first_sequence_never_moved(
    sync_db_engine: Engine, ledger: str
) -> None:
    ...
    _identity_rows(sync_db_engine, ledger, ids=(4,))
    config = monthly_config(ledger, create_ahead=1)
    make_service(sync_db_engine).partition_data(config)
    dest = _two_identity_destination(sync_db_engine, ledger)
    ...
    assert _count(sync_db_engine, ledger) == 1
    assert (
        scalar(sync_db_engine, f"SELECT pg_sequence_last_value(pg_get_serial_sequence('public.{dest}', 'id'))") is None
    )


def _one_connection_engine(postgres_container: PostgresContainer) -> Engine:
    """An engine with a single connection, so the caller and the library share a backend."""
    url = postgres_container.get_connection_url()
    if "://" in url:
        _, rest = url.split("://", 1)
        url = f"postgresql+psycopg2://{rest}"
    return create_engine(url, echo=False, pool_size=1, max_overflow=0)


def test__move_rows__a_temporary_table_of_the_callers_own__is_left_alone(
    sync_db_engine: Engine, ledger: str, postgres_container: PostgresContainer
) -> None:
    ...
    exec_sql(sync_db_engine, f'CREATE TABLE "{source}" (LIKE "{ledger}" INCLUDING ALL EXCLUDING IDENTITY)')
    exec_sql(
        sync_db_engine,
        f'INSERT INTO "{source}" (id, tenant_id, created_at, amount) '  # noqa: S608
        "VALUES (50, 1, make_timestamptz(2026, 4, 1, 12, 0, 0, 'UTC'), 1)",
    )
    dest = _identity_destination(sync_db_engine, ledger, "callertemp", "START WITH 100 CACHE 5")
    engine = _one_connection_engine(postgres_container)
    try:
        with engine.begin() as conn:
            conn.execute(text("CREATE TEMP TABLE pg_partsmith_moved_identity (id BIGINT)"))
            conn.execute(text("INSERT INTO pg_partsmith_moved_identity VALUES (999)"))

        # Act: the same backend now runs the move
        moved = PostgresPartitionRepository(engine).move_rows(source, dest)

        # Assert: the move parked its ids elsewhere and left the caller's table alone
        assert moved == 1
        with engine.begin() as conn:
            kept = (conn.execute(text("SELECT id FROM pg_partsmith_moved_identity"))).scalars().all()
