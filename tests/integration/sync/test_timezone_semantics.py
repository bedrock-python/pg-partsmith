...
def _partition_bound_expr(engine: Engine, partition: str) -> str:
...
def test__attach_partition__non_utc_session_timezone__boundaries_stored_in_utc(
    ...
    try:
        ...
        # Act
        repo.create_table_like(parent, partition, None)
        repo.attach_partition(parent, partition, RangeBounds(from_value="2024-01-01", to_value="2024-02-01"))
        ...
        expr = _partition_bound_expr(engine, partition)
...
def test__timezone__moscow_calendar__boundaries_are_moscow_midnights(
    postgres_container: PostgresContainer,
) -> None:
    ...
    try:
        ...
        # Act
        repo.create_table_like(parent, partition, None)
        repo.attach_partition(parent, partition, RangeBounds(from_value=from_value, to_value=to_value))
...
def test__timezone__moscow_calendar__pruning_selects_correct_set(
    postgres_container: PostgresContainer,
) -> None:
    # Arrange — the session TimeZone is unrelated to the configured DDL timezone.
    engine = _create_sync_engine(postgres_container, session_timezone="America/Los_Angeles")
...
def test__timezone__moscow_calendar__maintenance_creates_and_prunes_on_moscow_midnights(
    postgres_container: PostgresContainer,
) -> None:
    ...
    try:
        ...
        with freezegun.freeze_time("2024-05-31 22:00:00"):
            first = service.maintain(config)

        # Assert — the Moscow calendar decided both the name and the instant
        assert first.created_count == 1
        expr = _partition_bound_expr(engine, f"{parent}__2024_06")
        ...
        with freezegun.freeze_time("2024-07-15"):
            second = service.maintain(config)
...
def test__timezone__mismatched_pair__planning_is_refused(
    postgres_container: PostgresContainer,
) -> None:
    ...
    try:
        ...
        with pytest.raises(ValueError, match="Timezone mismatch"):
            service.plan(_moscow_config(table_relname))
        with pytest.raises(ValueError, match="Timezone mismatch"):
            service.maintain(_moscow_config(table_relname))
    finally:
        with engine.begin() as conn:
            conn.execute(text(f"DROP TABLE IF EXISTS {parent} CASCADE"))
        engine.dispose()
