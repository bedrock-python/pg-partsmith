"""Plans, dry runs, ownership and revalidation against a real PostgreSQL (async)."""
...
async def table(db_engine: AsyncEngine) -> AsyncGenerator[str, None]:
    async for name in make_table(db_engine, MONTHLY_TABLE_DDL, prefix="plan"):
        yield name
...
async def test__plan__fresh_table__lists_the_creations_and_issues_zero_ddl(db_engine: AsyncEngine, table: str) -> None:
    # Arrange
    config = monthly_config(table, create_ahead=3, retention=12)
    service = make_service(db_engine)

    # Act
    with freezegun.freeze_time(NOW), count_ddl(db_engine) as counter:
    ...
    assert await relkind(db_engine, f"{table}__2026_08") is None


async def test__maintain__after_plan__executes_exactly_that_plan_and_converges(
    ...
    with freezegun.freeze_time(NOW), count_ddl(db_engine) as counter:
        again = await service.maintain(config)
...
async def test__apply__of_a_plan_made_earlier__executes_it_under_the_lock(db_engine: AsyncEngine, table: str) -> None:
    # Arrange
    ...
    # Act
    result = await service.apply(config, plan.only(OperationKind.CREATE))

    # Assert
    assert result.created_count == 2
    assert result.maintenance_plan is not None
    assert result.maintenance_plan.operations == plan.operations
    assert await is_attached(db_engine, f"{table}__2026_08")
    assert await is_attached(db_engine, f"{table}__2026_09")


async def test__plan__json_dump__round_trips_through_model_validate(db_engine: AsyncEngine, table: str) -> None:
    ...
    await exec_sql(db_engine, f'CREATE TABLE "{table}_archive" (LIKE "{table}" INCLUDING ALL)')
    await exec_sql(
        db_engine,
...
async def test__plan__modes__reconcile_and_explicit_create_nothing_ahead(db_engine: AsyncEngine, table: str) -> None:
    # Arrange
    config = monthly_config(table, create_ahead=2, retention=1)
    await run_maintenance(db_engine, config, at_time="2026-06-15")  # June and July exist
    service = make_service(db_engine)

    # Act
    with freezegun.freeze_time(NOW):
        maintain = await service.plan(config)
        reconcile = await service.plan(config, mode=PlanMode.RECONCILE)
...
async def test__ownership__hand_attached_off_grid_partition__reported_as_unmanaged_and_never_touched(
    db_engine: AsyncEngine, table: str
) -> None:
    # Arrange: a DBA's archive spanning twenty years, on a monthly grid
    archive = f"{table}_archive"
    await exec_sql(db_engine, f'CREATE TABLE "{archive}" (LIKE "{table}" INCLUDING ALL)')
    await exec_sql(
        db_engine,
    ...
    for at_time in ("2026-09-15", "2026-10-15", "2027-01-15", "2027-06-15"):
        later = await run_maintenance(db_engine, config, at_time=at_time)
        assert later.issues == ()
    ...
    # Assert
    assert await is_attached(db_engine, archive) is True
    assert set(await range_children_of(db_engine, table)) == {archive, f"{table}__2027_06"}


async def test__ownership__hand_attached_partition_finer_than_the_grid__managed_and_expired_by_its_bound(
    db_engine: AsyncEngine, table: str
) -> None:
    # Arrange: a single day inside the monthly grid, left by an earlier daily config
    day = f"{table}_june_10"
    await exec_sql(db_engine, f'CREATE TABLE "{day}" (LIKE "{table}" INCLUDING ALL)')
    await exec_sql(
        db_engine,
    ...
    assert await relkind(db_engine, day) is None


async def test__ownership__wanted_window_overlapping_an_off_grid_partition__range_overlap_issue_and_no_ddl(
    db_engine: AsyncEngine, table: str
) -> None:
    # Arrange: a hand-attached window straddling two months, covering "now"
    wide = f"{table}_wide"
    await exec_sql(db_engine, f'CREATE TABLE "{wide}" (LIKE "{table}" INCLUDING ALL)')
    await exec_sql(
        db_engine,
    ...
    assert await is_attached(db_engine, wide) is True


# ── Destructive revalidation ────────────────────────────────────────────────────


async def _replace_orphan(engine: AsyncEngine, table: str, orphan: str) -> None:
    """Drop an orphan and recreate a same-named, same-marked table in its place."""
    await exec_sql(engine, f'DROP TABLE "{orphan}"')
    await exec_sql(engine, f'CREATE TABLE "{orphan}" (LIKE "{table}" INCLUDING ALL)')
    await exec_sql(engine, f"COMMENT ON TABLE \"{orphan}\" IS '{orphan_marker(f'public.{table}')}'")


async def test__apply__drop_of_a_relation_recreated_since_the_plan__refused_and_recorded_with_continue_on_error(
    db_engine: AsyncEngine, table: str
) -> None:
    # Arrange: June expired and was detached; a plan decides to drop it by OID
    config = monthly_config(table, create_ahead=1, retention=1)
    await run_maintenance(db_engine, config, at_time="2026-06-15")
    await run_maintenance(db_engine, config, at_time="2026-08-01", skip_drop=True)
    ...
    assert planned_oid == await relation_oid(db_engine, june)

    # A same-named, same-marked table takes its place before the plan is applied
    await _replace_orphan(db_engine, table, june)
    replacement_oid = await relation_oid(db_engine, june)
    ...
    assert await relation_oid(db_engine, june) == replacement_oid


async def test__apply__drop_of_a_relation_recreated_since_the_plan__raises_without_continue_on_error(
    db_engine: AsyncEngine, table: str
) -> None:
    # Arrange
    config = monthly_config(table, create_ahead=1, retention=1)
...
async def test__apply__detach_of_a_partition_detached_since_the_plan__refused_as_stale(
    db_engine: AsyncEngine, table: str
) -> None:
    ...
    await exec_sql(db_engine, f'ALTER TABLE "{table}" DETACH PARTITION "{june}"')

    # Act
    with freezegun.freeze_time(NOW):
        result = await service.apply(config, plan.without(OperationKind.CREATE), continue_on_error=True)
...
async def _foreign_leaf(
    ...
    *,
    ...
    async with db_engine.begin() as conn:
        await conn.execute(text("CREATE EXTENSION IF NOT EXISTS postgres_fdw"))
        await conn.execute(
            text(
        ...
        await conn.execute(text(f'CREATE TABLE "{remote}" (ts TIMESTAMPTZ NOT NULL, v DOUBLE PRECISION)'))
        await conn.execute(text(f"INSERT INTO \"{remote}\" VALUES ('{row_ts}', 1.5)"))  # noqa: S608
        await conn.execute(
            text(
    ...
    yield leaf
    async with db_engine.begin() as conn:
        await conn.execute(text(f"DROP SERVER IF EXISTS {server} CASCADE"))
        await conn.execute(text(f'DROP TABLE IF EXISTS "{remote}"'))


@pytest_asyncio.fixture
async def foreign_leaf(
    db_engine: AsyncEngine, metrics_table: str, postgres_container: PostgresContainer
) -> AsyncGenerator[str, None]:
    """The January 2025 partition, long behind every retention rule in these tests."""
    async for name in _foreign_leaf(
        db_engine,
        metrics_table,
        postgres_container,
        suffix="__2025_01",
...
async def test__foreign_leaf__covering_a_wanted_window__satisfies_it_without_an_overlap_or_a_create(
    db_engine: AsyncEngine, metrics_table: str, foreign_leaf_now: str
) -> None:
    # Arrange
    ...
    assert await is_attached(db_engine, foreign_leaf_now) is True


async def test__foreign_leaf__get_actual_tree__reports_the_foreign_relkind(
    db_engine: AsyncEngine, metrics_table: str, foreign_leaf: str
) -> None:
    # Arrange
    metadata = PostgresMetadataProvider(db_engine)

    # Act
    tree = await metadata.get_actual_tree(metrics_table)
...
async def test__foreign_leaf__plan__lists_a_foreign_partition_finding_and_no_operation_about_it(
    db_engine: AsyncEngine, metrics_table: str, foreign_leaf: str
) -> None:
...
async def test__foreign_leaf__maintain__never_detaches_or_drops_it(
    db_engine: AsyncEngine, metrics_table: str, foreign_leaf: str
) -> None:
    # Arrange
    config = monthly_config(metrics_table, create_ahead=1, retention=1, column="ts")
    ...
    second = await run_maintenance(db_engine, config, at_time="2026-10-15")
    ...
    assert await relkind(db_engine, foreign_leaf) == "f"
    assert await is_attached(db_engine, foreign_leaf) is True
...
async def test__foreign_leaf__detached_by_hand_and_marked__reported_and_never_dropped(
    db_engine: AsyncEngine, metrics_table: str, foreign_leaf: str
) -> None:
    ...
    config = monthly_config(metrics_table, create_ahead=1, retention=1, column="ts")
    await exec_sql(db_engine, f'ALTER TABLE "{metrics_table}" DETACH PARTITION "{foreign_leaf}"')
    await exec_sql(
        db_engine, f"COMMENT ON FOREIGN TABLE \"{foreign_leaf}\" IS '{orphan_marker(f'public.{metrics_table}')}'"
    )
    tree = await PostgresMetadataProvider(db_engine).get_actual_tree(metrics_table)
    assert tree is not None
...
async def test__plan__detach_mode__is_carried_on_the_operation(db_engine: AsyncEngine, table: str) -> None:
    # Arrange
