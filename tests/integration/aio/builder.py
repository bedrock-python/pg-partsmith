...
class PartitioningScenarioBuilder:
    ...
    async def build(self) -> PartitioningTestContext:
        ...
        for p in self._partitions:
            await repo.create_table_like(self._table_name, p.name, None)
            await repo.attach_partition(
                self._table_name, p.name, RangeBounds(from_value=p.from_value, to_value=p.to_value)
            )
            if not p.attached:
                with freezegun.freeze_time(p.to_value):
                    await repo.detach_partition(self._table_name, p.name, mode=DetachMode.BLOCKING)
