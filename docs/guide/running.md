...
15 2 * * *  docker run --rm -v /etc/pg-partsmith/partitions.yaml:/etc/partitions.yaml:ro \
            --env-file /etc/pg-partsmith/env ghcr.io/bedrock-python/pg-partsmith:latest \
...
ExecStart=/usr/bin/docker run --rm \
  ...
  -e PG_PARTSMITH_DSN ghcr.io/bedrock-python/pg-partsmith:latest \
...
docker service create --name partition-plan --restart-condition none --detach=false \
  ...
  ghcr.io/bedrock-python/pg-partsmith:latest plan -c /etc/partitions.yaml --locks
...
kubectl run partition-plan --rm -it --restart=Never \
  ...
  --overrides='{"spec":{"containers":[{"name":"pg-partsmith","image":"ghcr.io/bedrock-python/pg-partsmith:latest","args":["inspect","-c","/etc/partitions.yaml"],"volumeMounts":[{"name":"config","mountPath":"/etc/partitions.yaml","subPath":"partitions.yaml"}]}],"volumes":[{"name":"config","configMap":{"name":"partition-config"}}]}}'
```
...
```dockerfile
FROM ghcr.io/bedrock-python/pg-partsmith:latest
...
validate-partitions:
  ...
  before_script:
    - pip install --quiet "pg-partsmith[cli]"
  script:
    - pg-partsmith validate -c partitions.yaml
  variables:
