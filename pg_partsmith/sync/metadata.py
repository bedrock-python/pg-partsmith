...
class PostgresMetadataProvider:
    ...
    def _measure(
        ...
        if FactKind.REFERENCES in facts:
            ...
            foreign_keys = self._incoming_foreign_keys(referenced)
        ...
        for name, oid in oids.items():
            ...
            if FactKind.REFERENCES in facts:
                ...
                referenced_here = self._is_referenced(name, [fk for fk in foreign_keys if fk.referenced_oid in through])
    ...
    def _incoming_foreign_keys(self, oids: list[int]) -> list[_ForeignKey]:
        """Foreign keys of other tables pointing at any of ``oids``."""
        with self._engine.connect() as conn:
            result = conn.execute(text(INCOMING_FOREIGN_KEYS_SQL), {"oids": oids})
            rows = result.fetchall()
        return [
            _ForeignKey(
    ...
    def _is_referenced(self, partition_name: str, foreign_keys: list[_ForeignKey]) -> bool:
    ...
    def is_partition_closed(
        ...
        with self._engine.connect() as conn:
            if ddl_timezone is not None:
                ...
                conn.execute(text(f"SET LOCAL TIME ZONE {quote_literal(ddl_timezone)}"))
