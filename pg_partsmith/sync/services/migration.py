...
PlanForWindow = Callable[[Window], MaintenancePlan]
...
class DataMover:
    ...
    def partition_data(
        ...
        *,
        ...
        default = self._metadata.get_default_partition(config.qualified_name)
        if default is None:
            return MigrationResult(complete=True)

        tally = _Tally(max_batches)
        while True:
            probe = self._metadata.get_leading_key_minimum(default.name, root.key)
            if probe is None:
    ...
    def _require_destination(self, config: TablePartitionConfig, tree: ActualTree, into: str) -> None:
        ...
        if kind is not RelationKind.TABLE:
            ...
            raise InvalidPartitionConfigError(msg)
        if self._metadata.get_partition_tree(into) is not None:
    ...
    def _remove(self, config: TablePartitionConfig, child: PartitionNode, into: str, tally: _Tally) -> None:
        ...
        self._executor.detach_single_partition(config, detach)
        ...
        drained = self._executor.drop_single_partition(config, drop, drain_into=into)
        if drained:
    ...
    def _move_orphans(
        ...
        *,
        ...
        for orphan in sorted(tree.orphans, key=lambda candidate: candidate.name):
            ...
            if drop_emptied:
                self._drop_orphan(config, orphan, into, tally)
        return complete

    def _drop_orphan(self, config: TablePartitionConfig, orphan: DetachedPartition, into: str, tally: _Tally) -> None:
        drop = DropPartition(
