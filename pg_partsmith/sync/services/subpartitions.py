...
from pg_partsmith.sync.protocols import NestedPartitionMetadata, SubpartitionRepository
...
class PartitionSubpartitionService:
    ...
    def build_new_branch(self, config: TablePartitionConfig, branch_name: str) -> int:
    ...
    def converge_branch(self, config: TablePartitionConfig, branch_name: str) -> SubpartitionReconcileResult:
        ...
        """
        spec = self._require_spec(config)
        node = self._metadata.get_partition_tree(branch_name)
        if node is None:
            return SubpartitionReconcileResult()
        return self._apply(spec, node)
    ...
    def reconcile(
        ...
        *,
        ...
        tree = self._metadata.get_partition_tree(root)
        if tree is None:
            return SubpartitionReconcileResult()
        ...
        if config.root_layout is not None:
            ...
            return self._apply(config.root_layout, tree)
        ...
        for branch in tree.children:
            if not self._is_reconcilable(branch, exclude):
                continue
            result = result.merge(self._apply_isolated(spec, branch))

        return result

    def _apply_isolated(self, spec: SubpartitionSpec, node: PartitionNode) -> SubpartitionReconcileResult:
    ...
    def _apply(self, spec: SubpartitionSpec, node: PartitionNode) -> SubpartitionReconcileResult:
        ...
        created = self.materialize(plan.actions, execution)
        for finding in execution:
            _log_finding(finding)
        return SubpartitionReconcileResult(created_count=created, findings=plan.findings + tuple(execution))

    def _materialize_one(self, action: SubpartitionAction, findings: list[TopologyFinding]) -> int:
        ...
        created = 0
        try:
            self._repo.create_subpartition_table(action.parent_name, action.child_name, action.subpartition)
    ...
    def _attach(self, action: SubpartitionAction, findings: list[TopologyFinding]) -> bool:
        ...
        """
        try:
            self._repo.attach_subpartition(action.parent_name, action.child_name, action.bounds)
            ...
            if not self._matches_planned_bounds(action):
    ...
    def _matches_planned_bounds(self, action: SubpartitionAction) -> bool:
        ...
        """
        existing = self._metadata.get_partition_tree(action.child_name)
