...
class PartitionSubpartitionService:
        ...
        *,
        ...
        for branch in tree.children:
            if not self._is_reconcilable(branch, exclude):
                continue
            result = result.merge(self._apply(spec, branch))

        return result

    def materialize(self, actions: tuple[SubpartitionAction, ...]) -> int:
        ...
        for action in actions:
            created += self._materialize_one(action)
        return created
    ...
    def _apply(self, spec: SubpartitionSpec, node: PartitionNode) -> SubpartitionReconcileResult:
        ...
        created = self.materialize(plan.actions)
        return SubpartitionReconcileResult(created_count=created, findings=plan.findings)

    def _materialize_one(self, action: SubpartitionAction) -> int:
        ...
        created += self.materialize(action.children)

        if self._attach(action):
            created += 1
        return created

    def _attach(self, action: SubpartitionAction) -> bool:
        ...
        try:
            ...
            if not self._metadata.is_partition_attached(action.parent_name, action.child_name):
                raise
            logger.debug(
