...
class PartitionLifecycleService:
    ...
    * :meth:`maintain` — both, under one lock, which is what a scheduled tick
    ...
    def detach_old_partitions(self, config: TablePartitionConfig, partitions: list[PartitionInfo]) -> list[str]:
        ...
        for partition in partitions:
            ...
            self._executor.detach_single_partition(config, op)
            detached.append(partition.name)
        return detached

    def drop_detached_partitions(self, config: TablePartitionConfig, partition_names: list[str]) -> int:
        ...
        for name in partition_names:
            ...
            if self._executor.drop_single_partition(config, op) is not None:
                dropped += 1
        return dropped
