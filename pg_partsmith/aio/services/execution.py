...
class PlanExecutor:
    ...
    async def _create(
        ...
        *,
        ...
        try:
            ...
            resolved = await self._metadata.get_relation_oid(op.target)
            if resolved is None:
    ...
    async def _attach_detached(
        ...
        *,
        ...
        tally.repaired += await self._converge_detached(config, plan, op, depth=depth, issues=issues, parent_oid=op.oid)
        if fill is not None:
        ...
        for attempt in range(1, DEFAULT_CONFLICT_MAX_RETRIES + 1):
            try:
                if default_partition is None or window is None:
                    ...
                    moved = await self._repo.reconcile_and_attach(
                        parent_name,
                        partition_name,
                        window,
