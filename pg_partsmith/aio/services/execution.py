...
class PlanExecutor:
    ...
    async def create_partition(
        ...
        *,
        ...
        fill: Callable[[str, int | None], Awaitable[bool]] | None = None,
    ) -> bool:
    ...
    async def _create(
        ...
        *,
        ...
        try:
            target_oid = await self._require_identity(op.target, await self._materialize(config, op))
            ...
            target_oid = await self._require_identity(op.target, await self._metadata.get_relation_oid(op.target))
            tally.repaired += await self._converge_detached(
        ...
        if fill is not None and not await fill(op.target, target_oid):
            return False
    ...
    async def _require_identity(self, name: str, oid: int | None) -> int:
        ...
        if oid is not None:
            return oid
        resolved = await self._metadata.get_relation_oid(name)
        if resolved is None:
    ...
    async def _materialize(self, config: TablePartitionConfig, op: CreatePartition) -> int | None:
        ...
        if op.partition_by is None and isinstance(leaves, ForeignLeaves):
            ...
            return await self._repo.create_foreign_table_like(
                op.parent_name, op.target, server=leaves.server, options=options
            )
        if isinstance(leaves, LocalLeaves) and not leaves.is_plain:
            return await self._repo.create_table_like(op.parent_name, op.target, op.partition_by, physical=leaves)
        ...
        return await self._repo.create_table_like(op.parent_name, op.target, op.partition_by)
    ...
    async def _attach_detached(
        ...
        *,
        ...
        if fill is not None:
            if not await fill(op.target, op.oid):
                return False
        ...
        *,
        ...
        for attempt in range(1, DEFAULT_CONFLICT_MAX_RETRIES + 1):
            try:
                ...
                await self._restore_reconciled_rows(reconciled_from, partition_name, expected_oid, window, key_columns)
                raise
            except SQLAlchemyError as exc:
                if pg_sqlstate(exc) in ATTACH_CONFLICT_SQLSTATES:
                    await self._restore_reconciled_rows(
                        reconciled_from, partition_name, expected_oid, window, key_columns
                    )
